...
class MpesasController < ApplicationController
  ...
  class << self
    attr_reader :mpesa_consumer_key, :mpesa_consumer_secret, :mpesa_passkey,
                :mpesa_initiator_username, :mpesa_initiator_security_credential,
                :callback_url, :query_url, :register_url, :mpesa_shortcode
  end
  ...
  def stkpush
    ...
    business_short_code = self.class.mpesa_shortcode # Get the Mpesa business short code from class instance variable
    password = Base64.strict_encode64("#{business_short_code}#{self.class.mpesa_passkey}#{timestamp}") # Generate the password for the request
    ...
    payload = {
      ...
      'CallBackURL': "#{self.class.callback_url}/callback_url", # URL for receiving callback from Mpesa
  ...
  def polling_payment
    ...
    url = self.class.query_url # URL for querying payment status
  ...
  def generate_access_token_request
    ...
    consumer_key = self.class.mpesa_consumer_key # Get the consumer key from class instance variable
    consumer_secret = self.class.mpesa_consumer_secret # Get the consumer secret from class instance variable
